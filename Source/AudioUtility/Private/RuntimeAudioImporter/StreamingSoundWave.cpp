...
#include "RuntimeAudioImporter/StreamingSoundWave.h"
...
{
	{
		...
		{
			UE_LOG(AudioLog, Error, TEXT("Unable to continue populating the audio data because the decoded info is invalid"));
			return;
		}
		...
		{
			UAudioImporter::ResampleAndMixChannelsInDecodedInfo(DecodedAudioInfo, SampleRate, NumChannels);
		}
	...
	UE_LOG(AudioLog, Verbose, TEXT("Successfully added audio data to streaming sound wave.\nAdded audio info: %s"), *DecodedAudioInfo.ToString());
}
...
void UStreamingSoundWave::PreAllocateAudioData(int64 NumOfBytesToPreAllocate, const FOnPreAllocateAudioDataResultNative& Result)
{
	if (IsInGameThread())
	{
		AsyncTask(ENamedThreads::AnyBackgroundHiPriTask, [WeakThis = MakeWeakObjectPtr(this), NumOfBytesToPreAllocate, Result] ()
		{
			...
			{
				UE_LOG(AudioLog, Error, TEXT("Failed to pre-allocate streaming sound wave audio data as the streaming sound wave has been destroyed"));
			}
		});
		return;
	}
	...
	UE_LOG(AudioLog, Log, TEXT("Successfully pre-allocated '%lld' number of bytes"), NumOfBytesToPreAllocate);
	ExecuteResult(true);
}
...
void UStreamingSoundWave::AppendAudioDataFromEncoded(TArray<uint8> AudioData, ERuntimeAudioFormat AudioFormat)
{
	if (IsInGameThread())
	{
		AudioTaskPipe->Launch(AudioTaskPipe->GetDebugName(), [WeakThis = MakeWeakObjectPtr(this), AudioData = MoveTemp(AudioData), AudioFormat] () mutable
		{
			...
			{
				UE_LOG(AudioLog, Error, TEXT("Failed to append audio data to streaming sound wave as the streaming sound wave has been destroyed"));
			}
	...
	if (!UAudioImporter::DecodeAudioData(MoveTemp(EncodedAudioInfo), DecodedAudioInfo))
	{
		UE_LOG(AudioLog, Error, TEXT("Failed to decode audio data to populate streaming sound wave audio data"));
		return;
	}
...
void UStreamingSoundWave::AppendAudioDataFromRAW(TArray<uint8> RAWData, ERuntimeRAWAudioFormat RAWFormat, int32 InSampleRate, int32 NumOfChannels)
{
	if (IsInGameThread())
	{
		AudioTaskPipe->Launch(AudioTaskPipe->GetDebugName(), [WeakThis = MakeWeakObjectPtr(this), RAWData = MoveTemp(RAWData), RAWFormat, InSampleRate, NumOfChannels] () mutable
		{
			...
			{
				UE_LOG(AudioLog, Error, TEXT("Failed to append RAW audio data to streaming sound wave as the streaming sound wave has been destroyed"));
			}
		}, UE::Tasks::ETaskPriority::BackgroundHigh);
		return;
	}
	...
	{
		UE_LOG(AudioLog, Error, TEXT("Failed to transcode RAW data to decoded audio info"))
			return;
	}
